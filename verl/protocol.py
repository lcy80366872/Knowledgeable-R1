...
class DataProto:
    ...
    @classmethod
    def from_single_dict_aug(cls, data: Dict[str, Union[torch.Tensor, NDArray]], meta_info=None):
        tensors = {}
        non_tensors = {}
        
        # 处理所有以 _aug 结尾的键
        keys_to_process = [key for key in data.keys() if key.endswith("_aug")]
        for key in keys_to_process:
            # 移除 _aug 后缀
            print("key",key)
            new_key = key.replace("_aug", "")
            # 移动数据到新键
            data.pop(new_key)
    ...
    @staticmethod
    def concat(data: List["DataProto"], interleave: bool = False) -> "DataProto":
    ...
    def interleave_batches(self, num_segments=2):
        """
        实现了一个批处理数据交错重排的功能
        Reorder the batch data in an interleaved pattern.
        For example, with num_segments=2:
        [1,2,3,4,5,6] -> [1,4,2,5,3,6]
        
        Args:
            num_segments (int): Number of segments to interleave.
        
        Returns:
            DataProto: A new DataProto with interleaved data.
        """
        batch_size = len(self)
        assert batch_size % num_segments == 0, f"Batch size {batch_size} must be divisible by num_segments {num_segments}"
        ...
        for i in range(segment_size):
            for j in range(num_segments):
                indices[i * num_segments + j] = j * segment_size + i
        
        # Create a new DataProto with reordered data
        if self.batch is not None:
            reordered_batch = self.batch[indices]
        else:
            reordered_batch = None
        
        # Handle non-tensor batch data
        indices_np = indices.numpy()
        reordered_non_tensor_batch = {key: val[indices_np] for key, val in self.non_tensor_batch.items()}
